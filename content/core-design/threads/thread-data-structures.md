...
4. **Contract-first ADT teaching, confirmed (2026-08-03).** CIS 300's actual week-by-week schedule bears this out directly: applications precede implementations at every ADT pass (Stack Applications wk2 → Stack Implementation wk3; Queue Fundamentals/Applications wk3 → Queue Implementations wk4; Dictionaries wk6 → Dictionary Implementations wk7).
5. **Spatial indexing (quadtrees, R-trees, geohashing) — not actually in CIS 300, confirmed a gap (2026-08-03).** Neither the real CIS 300 schedule nor `cs.md`'s degree-map entry for CIS 300 ever included this content; the claim in this thread's pass table was aspirational, not built. Logged as a proposed addition in `cis-300.md`'s own "Proposed Changes" section pending a course-designer/faculty placement decision.
6. **CIS 200's collections/iterators touch, added 2026-08-03 — confirm it stays genuinely informal.** CIS 200's SLO 5 ("collection classes and iterators") is real, confirmed content, but its course schedule is still TBD, so there's no way yet to confirm it stays a light, informal preview rather than drifting into territory CIS 300 is supposed to formalize a semester later. Revisit once CIS 200's schedule is drafted.
